...
export const metadata = {
  ...
  faq: [
    {
      // Phrased as the literal head term so the FAQPage entry matches the
      // query verbatim, not just semantically. See TRZ-1586.
      question: "Context layer vs RAG: what's the difference?",
      answer:
        "A context layer and RAG operate at different levels of the stack. RAG is a retrieval tactic — embed a query, fetch the nearest chunks, paste them into the prompt. A context layer is an architectural tier that sits between Retrieval and Inference and turns whatever was retrieved into decision-ready context: curating it, synthesizing across sources, consolidating conflicts, prioritizing by the decision at hand, and storing the result. RAG optimizes what gets fetched. The context layer determines what the fetched material means. A system can have excellent RAG and no context layer at all — that is the most common failure mode in production AI products.",
    },
    {
