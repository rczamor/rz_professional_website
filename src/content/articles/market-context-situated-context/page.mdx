...
Context architecture becomes product strategy when the system learns how decisions actually happen inside each customer.

<SectionLabel>01 / The Category Error</SectionLabel>

## The industry has been talking about the wrong thing.

Inside the AI industry, the word context has drifted. Ask ten practitioners what context means right now and nine of them will answer with some mix of context windows, agent memory, retrieval architectures, vector stores, KV cache, token budgets, and RAG patterns. Context has become a synonym for the stuff you put into the model — which is precisely the confusion that makes [context layer vs RAG](/thinking/context-layer-vs-rag) a question worth answering.
...
Notice what's happening in that pipeline. None of those five moves are about the context window. None of them are about retrieval latency. They're about turning raw signal into situated understanding. The technology is necessary, but it's the scaffolding, not the work. I made the architectural version of that argument separately: [what a Context Layer actually is, and why agent memory isn't one](/thinking/what-is-a-context-layer).
