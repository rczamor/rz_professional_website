...
It's the same discipline I apply to context patterns in AI systems. [RAG isn't good or bad](/thinking/data-is-not-context); it's good under specific conditions, and the job is knowing whether you have them. That is the whole argument behind [context layer vs RAG](/thinking/context-layer-vs-rag): the retrieval tactic is fine, the missing architecture around it is not. Engineers implement the pattern. Architects decide which pattern serves which preconditions — user, workflow, business. A method, a memory architecture, a retrieval strategy: each one is downstream of the conditions it was born into, and adopting it blind is the same mistake every time.
